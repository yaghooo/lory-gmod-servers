...
function GM:RadialMousePressed(code, vec)
    if radialOpen then
        ...
        if selected and selected > 0 and code == MOUSE_LEFT and ments[selected] then
...
function GM:DrawRadialMenu()
    if radialOpen then
        ...
        if not circleVertex then
            ...
            for i = 0, max do
                local vx, vy = math.cos((pi * 2) * i / max), math.sin((pi * 2) * i / max)

                table_insert(circleVertex, {
                    x = sx + w * 1 * vx,
                    y = sy + h * 1 * vy
                })
            end
        ...
        for k, ment in pairs(ments) do
            local x, y = math.cos((k - 1) / total * pi * 2 + pi * 1.5), math.sin((k - 1) / total * pi * 2 + pi * 1.5)
            local lx, ly = math.cos((k - 1) / total * pi * 2 + add), math.sin((k - 1) / total * pi * 2 + add)
            ...
            if selected == k then
                ...
                if prevSelected ~= selected then
                    ...
                    local lx2, ly2 = math.cos((k - 1) / total * pi * 2 + add2), math.sin((k - 1) / total * pi * 2 + add2)

                    table_insert(vertexes, {
                        x = sx,
                        y = sy
                    })
                    ...
                    for i = 0, max do
                        ...
                        local vx, vy = math.cos((k - 1) / total * pi * 2 + addv), math.sin((k - 1) / total * pi * 2 + addv)
