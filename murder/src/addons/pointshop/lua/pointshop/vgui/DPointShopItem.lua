...
local canbuycolor = Color(40, 40, 40)
...
function PANEL:DoClick()
    ...
    if self.IsInventory then
        ...
        menu:AddOption("Vender", function()
            Derma_Query("Tem certeza que quer vender " .. self.Data.Name .. "?", "Vender item", "Yes", function()
    ...
    elseif client:PS_HasPoints(points) then
        ...
        menu:AddOption("Comprar", function()
            Derma_Query("Tem certeza que quer comprar " .. self.Data.Name .. "?", "Comprar item", "Yes", function()
    ...
    if self.IsInventory and self.Data.CanPlayerEquip then
        ...
        if client:PS_HasItemEquipped(self.Data.ID) then
            menu:AddOption(self.Data.EquipLabel or "Desequipar", function()
                client:PS_HolsterItem(self.Data.ID)
            end)
        else
            menu:AddOption(self.Data.EquipLabel or "Equipar", function()
...
function PANEL:SetData(data, category, isInventory)
    ...
    if data.Model then
        ...
        if data.ImgurImage then
            ...
            PS:GetImageMaterial(data.ImgurImage, function(material)
...
function PANEL:PaintOver()
    ...
    if client.PS_Items and client.PS_Items[self.Data.ID] and client.PS_Items[self.Data.ID].Modifiers and client.PS_Items[self.Data.ID].Modifiers.color then
