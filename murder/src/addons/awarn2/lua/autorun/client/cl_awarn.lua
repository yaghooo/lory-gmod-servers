...
function awarn_optionsmenu()
    ...
    MenuPanelSlider4.TextArea:SetPaintBackground(true)
...
net.Receive("SendOwnWarns", function(length)
    ...
    if IsValid(AWarn.ClientMenuFrame) then
...
net.Receive("AWarnMenu", function(length)
    if not IsValid(AWarn.MenuFrame) then
        awarn_menu()
    end
end)

net.Receive("AWarnClientMenu", function(length)
    if not IsValid(AWarn.ClientMenuFrame) then
        awarn_clientmenu()
    end
end)
