...
if ULib.fileExists("lua/ulx/modules/cl/motdmenu.lua") or ulx.motdmenu_exists then
	local function sendMotd(ply, showMotd)
		if ply.ulxHasMotd then
			return
		end -- This player already has the motd data
		if showMotd == "1" then -- Assume it's a file
			if not ULib.fileExists(GetConVarString("ulx_motdfile")) then
				return
			end -- Invalid
	...
	local function conVarUpdated(sv_cvar, cl_cvar, ply, old_val, new_val)
		if
			string.lower(cl_cvar) == "ulx_showmotd" or string.lower(cl_cvar) == "ulx_motdfile" or
				string.lower(cl_cvar) == "ulx_motdurl"
		 then
			ulx.motdUpdated()
		end
	end
	...
	if SERVER then
		...
		function ulx.populateMotdData()
			if ulx.motdSettings == nil or ulx.motdSettings.info == nil then
				return
			end
