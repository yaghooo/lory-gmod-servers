...
function atlaschat.sql.Initialize(callback_success, callback_failed)
    ...
    if not exists then
        sql.Query(
            "CREATE TABLE " ..
                tableName ..
                    '(address TEXT DEFAULT "" NOT NULL UNIQUE, port INTEGER DEFAULT 3306 NOT NULL UNIQUE, user TEXT DEFAULT "" NOT NULL UNIQUE, password TEXT DEFAULT "" NOT NULL UNIQUE, database TEXT DEFAULT "atlaschat" NOT NULL UNIQUE)'
        )
    end
    ...
    if data then
        ...
        if mysqloo then
            if atlaschat.sql.remote.object then
                atlaschat.sql.remote.object = nil
            end
    ...
    if atlaschat.sql.remote.object then
        ...
        function queryObject:onError(message, queryString)
            ...
            if status == mysqloo.DATABASE_NOT_CONNECTED then
                table.insert(
                    queue,
                    {query = query, callback_success = callback_success, callback_failed = callback_failed}
                )
                                            ...
                                            port ..
                                                ", user = " ..
                                                    sql.SQLStr(username) ..
                                                        ", password = " ..
                                                            sql.SQLStr(password) ..
                                                                ", database = " .. sql.SQLStr(database)
                    )
                else
                    sql.Query(
                        "INSERT INTO " ..
                            tableName ..
                                "(address, port, user, password, database) VALUES(" ..
                                    sql.SQLStr(address) ..
                                        ", " ..
                                            port ..
                                                ", " ..
                                                    sql.SQLStr(username) ..
                                                        ", " ..
                                                            sql.SQLStr(password) .. ", " .. sql.SQLStr(database) .. ")"
                    )
                end
