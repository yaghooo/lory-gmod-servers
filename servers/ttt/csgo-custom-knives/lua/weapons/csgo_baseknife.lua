...
if (SERVER) then
    ...
    CreateConVar(
        "csgo_knives_force_ttt",
        0,
        FCVAR_ARCHIVE,
        "Forces knives to enable TTT mode. For debug purposes. Normally you shouldn't enable it unless you haven't any trouble getting it work in ttt"
    )
    ...
    CreateConVar(
        ...
        1,
        FCVAR_ARCHIVE,
        "Allow buying knives from traitor shop in TTT. May require server restarting if changing"
    )

    CreateConVar(
        "csgo_knives_dmg_sec_back",
        180,
        FCVAR_ARCHIVE,
        "How much damage deal when hit with secondary attack from behind"
    )
    CreateConVar(
        "csgo_knives_dmg_sec_front",
        65,
        FCVAR_ARCHIVE,
        "How much damage deal when hit with secondary attack in front or from side"
    )
    CreateConVar(
        "csgo_knives_dmg_prim_back",
        90,
        FCVAR_ARCHIVE,
        "How much damage deal when hit with primary attack from behind"
    )
    CreateConVar(
        "csgo_knives_dmg_prim_front1",
        40,
        FCVAR_ARCHIVE,
        "How much damage deal when firstly hit with primary attack in front or from side"
    )
    CreateConVar(
        "csgo_knives_dmg_prim_front2",
        25,
        FCVAR_ARCHIVE,
        "How much damage deal when subsequently hit with primary attack in front or from side"
    )
end

if (CLIENT) then
    CreateClientConVar(
        "cl_csgo_knives_lefthanded",
        "0",
        true,
        false,
        "Flip knives viewmodel and hold knives on left hand"
    )
...
function SWEP:PrimaryAttack()
    ...
    if not (prim or sec) or (CurTime() < self.Weapon:GetNextPrimaryFire()) then
        return
    end
...
function SWEP:SecondaryAttack()
    local prim = cvars.Bool("csgo_knives_primary", true)
    local sec = cvars.Bool("csgo_knives_secondary", true)
    if not (prim or sec) or (CurTime() < self.Weapon:GetNextSecondaryFire()) then
        return
    end
...
function SWEP:DoAttack(Altfire)
    ...
    local Damage =
        (Altfire and (Backstab and RMB_BACK or RMB_FRONT)) or (Backstab and LMB_BACK) or (FirstHit and LMB_FRONT1) or
        LMB_FRONT2
    ...
    local Act =
        DidHit and
        (Altfire and (Backstab and ACT_VM_SWINGHARD or ACT_VM_HITCENTER2) or
            (Backstab and ACT_VM_SWINGHIT or ACT_VM_HITCENTER)) or
        (Altfire and ACT_VM_MISSCENTER2 or ACT_VM_MISSCENTER)
    if Act then
