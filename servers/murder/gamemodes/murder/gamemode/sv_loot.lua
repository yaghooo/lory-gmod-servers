...
function GM:LoadLootData()
    ...
    local json = file.ReadDataAndContent("murder/" .. mapName .. "/loot.txt")
...
function GM:LootThink()
    if self:GetRound() == 1 and (not self.NextSpawnLoot or self.NextSpawnLoot < CurTime()) then
        self.NextSpawnLoot = CurTime() + 12
...
function GM:SaveLootData()
    ...
    -- JSON!
    local json = util.TableToJSON(LootItems)
    file.Write("murder/" .. mapName .. "/loot.txt", json)
end
