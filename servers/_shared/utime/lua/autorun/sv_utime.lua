...
if not sql.TableExists("utime") then
	sql.Query(
		"CREATE TABLE IF NOT EXISTS utime ( id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, player STRING NOT NULL, totaltime INTEGER NOT NULL, lastvisit INTEGER NOT NULL );"
	)
...
function updatePlayer(ply)
	sql.Query(
		"UPDATE utime SET totaltime = " ..
			math.floor(ply:GetUTimeTotalTime()) .. " WHERE player = '" .. ply:SteamID64() .. "';"
	)
end
