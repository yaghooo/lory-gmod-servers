...
local function steamIDHasAccess(steamid, priv, callback, targetPly, extra)
	...
	if connectedPly then
		return playerHasAccess(connectedPly, priv, callback, targetPly, extra)
	end
...
local function onGroupRegistered(camiGroup, originToken)
	...
	if originToken == CAMI.ULX_TOKEN then
		return
	end
	if ULib.findInTable({"superadmin", "admin", "user"}, camiGroup.Name) then
		return
	end
...
local function onUsersGroupChanged(ply, oldGroup, newGroup, originToken)
	if not ply or not ply:IsValid() then
		return
	end -- Seems like we get called after a player disconnects sometimes
	if originToken == CAMI.ULX_TOKEN then
		return
	end
