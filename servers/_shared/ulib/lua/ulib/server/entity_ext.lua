...
local function physgunReload(weapon, ply)
	...
	local ent = tr.Entity
	if not ent or not ent:IsValid() or ent:IsWorld() then
		return
	end -- Invalid or not interested
	if ent.NoMoving then
		return false
	end
end
...
local function removedCheck(ent)
	if ent.NoDeleting and not ent.NoReplication then
		...
		ULib.queueFunctionCall(
			function()
				-- Create it next frame because 1. Old ent won't be in way and 2. We won't overflow the server while shutting down
