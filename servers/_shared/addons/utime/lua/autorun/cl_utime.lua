...
function think()
	if not LocalPlayer():IsValid() or gpanel == nil then
		return
	end

	if
		not utime_enable:GetBool() or not IsValid(LocalPlayer()) or
	...
	gpanel:SetPos(
		(ScrW() - gpanel:GetWide()) * utime_pos_x:GetFloat() / 100,
		(ScrH() - gpanel.Large) * utime_pos_y:GetFloat() / 100
	)
