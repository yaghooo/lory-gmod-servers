...
local function voteMapDone(t, argv, ply)
	...
	elseif ply:IsValid() then
		...
		ulx.doVote(
			"Accept result and changemap to " .. changeTo .. "?",
			{"Yes", "No"},
			voteMapDone2,
			30000,
			{ply},
...
end -- The ratio needed for a votemap2 to succeed
if SERVER then
	ulx.convar("votemap2Minvotes", "3", _, ULib.ACCESS_ADMIN)
end -- Minimum votes needed for votemap2
...
local function voteKickDone(t, target, time, ply, reason)
	...
	if winner ~= 1 or winnernum < minVotes or winnernum / t.voters < ratioNeeded then
		...
		elseif ply:IsValid() then
			...
			ulx.doVote(
				"Accept result and kick " .. target:Nick() .. "?",
				{"Yes", "No"},
				voteKickDone2,
				30000,
				{ply},
...
end -- The ratio needed for a votekick to succeed
if SERVER then
	ulx.convar("votekickMinvotes", "2", _, ULib.ACCESS_ADMIN)
end -- Minimum votes needed for votekick
...
end -- The ratio needed for a voteban to succeed
if SERVER then
	ulx.convar("votebanMinvotes", "3", _, ULib.ACCESS_ADMIN)
end -- Minimum votes needed for voteban
