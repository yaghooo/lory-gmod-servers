...
xlib.makecheckbox {
	...
	repconvar = "rep_sbox_playershurtplayers",
	parent = sbox_settings
}
...
xlib.makecheckbox {
	x = 10,
	y = 150,
	label = "Bone Manip. Misc",
	repconvar = "rep_sbox_bonemanip_misc",
	parent = sbox_settings
}
xlib.makecheckbox {
	x = 10,
	y = 170,
	label = "Bone Manip. NPC",
	repconvar = "rep_sbox_bonemanip_npc",
	parent = sbox_settings
}
xlib.makecheckbox {
	x = 10,
	y = 190,
	label = "Bone Manip. Player",
	repconvar = "rep_sbox_bonemanip_player",
	parent = sbox_settings
}
...
xlib.makelabel {
	...
	w = 140,
	wordwrap = true,
	label = "NOTE: The non-ulx cvars configurable in XGUI are provided for easy access only and DO NOT SAVE when the server is shut down or crashes.",
	parent = sbox_settings
}
sbox_settings.plist =
	xlib.makelistlayout {x = 140, y = 5, h = 322, w = 440, spacing = 1, padding = 2, parent = sbox_settings}
		...
		if #limits > 0 then
			...
			for _, cvar in ipairs(limits) do
				...
				xgui.queueFunctionCall(
					...
					{
						x = 10 + (i % 2 * 205),
						y = 5 + math.floor(i / 2) * 25,
						w = 200,
						label = "Max " .. cvardata[1]:sub(9),
						min = 0,
						max = cvardata[2],
						repconvar = "rep_" .. cvardata[1],
						parent = panel,
						fixclip = true
					}
				)
			...
			sbox_settings.plist:Add(
				xlib.makecat {
					label = limits.title .. " (" .. #limits .. " limit" .. ((#limits > 1) and "s" or "") .. ")",
					contents = panel,
					expanded = (g == 1)
				}
			)
