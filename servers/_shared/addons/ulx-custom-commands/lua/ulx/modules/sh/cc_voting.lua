...
local function voteGagDone(t, target, time, ply)
	...
	if winner ~= 1 or winnernum < minVotes or winnernum / t.voters < ratioNeeded then
		...
		str =
			"Vote results: User will now be gagged for " ..
			time .. " minutes, pending approval. (" .. winnernum .. "/" .. t.voters .. ")"

		ulx.doVote(
			"Accept result and gag " .. target:Nick() .. "?",
			{"Yes", "No"},
			voteGagDone2,
			30000,
			{ply},
...
if SERVER then
	ulx.convar("votegagSuccessratio", "0.7", _, ULib.ACCESS_ADMIN)
end
if SERVER then
	ulx.convar("votegagMinvotes", "3", _, ULib.ACCESS_ADMIN)
end
...
local function voteMuteDone(t, target, time, ply)
	...
	if winner ~= 1 or winnernum < minVotes or winnernum / t.voters < ratioNeeded then
		...
		str =
			"Vote results: User will now be muted for " ..
			time .. " minutes, pending approval. (" .. winnernum .. "/" .. t.voters .. ")"

		ulx.doVote(
			"Accept result and mute " .. target:Nick() .. "?",
			{"Yes", "No"},
			voteMuteDone2,
			30000,
			{ply},
...
if SERVER then
	ulx.convar("votemuteSuccessratio", "0.7", _, ULib.ACCESS_ADMIN)
	ulx.convar("votemuteMinvotes", "3", _, ULib.ACCESS_ADMIN)
end
