...
hook.Add(
    "PostGamemodeLoaded",
    "atlaschat.AddGamemodeFunctions",
    function()
        ----------------------------------------------------------------------
        -- Purpose:
...
hook.Add(
    "PlayerBindPress",
    "atlaschat.PlayerBindPress",
    function(player, bind, pressed)
...
hook.Add(
    "player_disconnect",
    "atlaschat.DisconnectMessage",
    function(data)
        local filtered = atlaschat.filterJoinDisconnect:GetBool()

        if string.find(data.reason, "timed out") then
        ...
        elseif string.find(data.reason, "Karma too low") then
            data.reason = "Banido por 2 horas: Karma muito baixo"
        elseif string.find(data.reason, "Changed name during a round") then
            data.reason = "Banido por 10 minutos: Mudou de nome durante uma rodada"
        elseif string.find(data.reason, "Kicked") then
            data.reason = "Kickado"
        else
            data.reason = ""
            return
...
hook.Add(
    "ChatText",
    "atlaschat.ChatText",
    function(index, name, text, filter)
...
if not atlaschat.chatAddText then
    atlaschat.chatAddText = chat.AddText
end
...
local function __clean(s, replacement)
    ...
    while p <= len do
        ...
        elseif p == s:find("[\194-\223][\128-\191]", p) then
            p = p + 2
        elseif
            p == s:find("\224[\160-\191][\128-\191]", p) or p == s:find("[\225-\236][\128-\191][\128-\191]", p) or
                p == s:find("\237[\128-\159][\128-\191]", p) or
                p == s:find("[\238-\239][\128-\191][\128-\191]", p)
         then
            p = p + 3
        elseif
            p == s:find("\240[\144-\191][\128-\191][\128-\191]", p) or
                p == s:find("[\241-\243][\128-\191][\128-\191][\128-\191]", p) or
                p == s:find("\244[\128-\143][\128-\191][\128-\191]", p)
         then
            p = p + 4
        else
...
function atlaschat.BuildFontCache(text, font)
    ...
    if not atlaschat.fontCache[lowered] then
        atlaschat.fontCache[lowered] = {}
    end
...
function atlaschat.NewBasePanel()
    ...
    function panel:Think()
        ...
        if IsValid(chatPanel) and chatPanel:IsVisible() then
            ...
            if x >= 0 and y >= 0 and x <= self:GetWide() and y <= self:GetTall() then
                ...
                if x >= 0 and x <= childrenWidth + 8 then
                    ...
                    for k, child in pairs(children) do
                        if IsValid(child) and not child.cursor then
                    ...
                    if input.IsMouseDown(MOUSE_LEFT) then
                        ...
                        if not IsValid(previousPanel) then
                            previousPanel = self
                        else
                                ...
                                if IsValid(child) then
                                    ...
                                    if x >= x2 and x <= x2 + child:GetWide() then
                                        if child:GetClassName() == "Label" then
                                            local text, start, font, totalWidth =
                                                child:GetText(),
                                                1,
                                                string.lower(child:GetFont()),
                                                0
                                            local len = string.utf8len(text)
                                    ...
                                    if x >= x2 and x <= x2 + wide then
                                        if child:GetClassName() == "Label" then
                                            ...
                                            while start <= len do
                                                ...
                                                if
                                                    start > len and x2 + totalWidth + width >= x2 + wide and
                                                        x >= x2 + totalWidth
                                                 then
        ...
        if IsValid(chatPanel) and not chatPanel:IsVisible() then
                    ...
                    function()
                        ...
                        for i = 1, #sortedChildren do
                            ...
                            if (x >= x2 or x2 >= x) and (width <= x2 + wide or width >= x2 + wide) then
                                if child:GetClassName() == "Label" then
                                    local text, start, font, totalWidth, final =
                                        child:GetText(),
                                        1,
                                        string.lower(child:GetFont()),
                                        0,
...
hook.Add(
    "Think",
    "atlaschat.TextSelection",
    function()
        local panel = atlaschat.theme.GetValue("panel")

        if input.IsKeyDown(KEY_ESCAPE) and IsValid(panel) and panel:IsVisible() then
        ...
        if
            input.IsKeyDown(KEY_LCONTROL) and input.IsKeyDown(KEY_C) and IsValid(previousPanel) and
                IsValid(currentPanel)
         then
...
function atlaschat.GenericLabel()
    ...
    function label:Paint(w, h)
        local shadow, text, font, extraShadow, pcall =
            shadow,
            self:GetText(),
            self:GetFont(),
            atlaschat.extraShadow:GetBool(),
            pcall
...
net.Receive(
    ...
    function(bits)
        ...
        chat.AddText(
            ...
            player:Nick(),
            color_white,
            " has invited you to a private chat. Click this to ",
            accept
        )
    end
)
...
net.Receive(
    ...
    function(bits)
        ...
        if LocalPlayer():IsSuperAdmin() then
            ...
            if IsValid(rankMenu) and rankMenu:IsVisible() then
                if special == 2 then
