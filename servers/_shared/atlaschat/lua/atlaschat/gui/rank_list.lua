...
function panel:Init()
    ...
    function self.button:OnMousePressed()
        Derma_StringRequest(
            "Add Usergroup",
            "Enter the name/unique of the usergroup",
            "",
            function(text)
                if text ~= "" then
                    net.Start("atlaschat.crtrnk")
...
function panel:Populate()
    ...
    for unique, data in pairs(atlaschat.ranks) do
        ...
        function usergroup.button:OnMousePressed()
            Derma_Query(
                ...
                function()
                    ...
                    net.WriteString(usergroup.text)
                    net.SendToServer()
                end,
                "Cancel",
                function()
        ...
        timer.Simple(
            0.1,
            function()
                atlaschat.ParseExpressionPreview(tag, "DermaDefault", true)
            end
        )
...
function panel:Paint(w, h)
    draw.SimpleText(
        self.text,
        "atlaschat.ranks.column",
        self.align_x == TEXT_ALIGN_CENTER and w / 2 or self.align_x == TEXT_ALIGN_RIGHT and w - 3 or 4,
        0,
        color_column,
        self.align_x,
        TEXT_ALIGN_TOP
