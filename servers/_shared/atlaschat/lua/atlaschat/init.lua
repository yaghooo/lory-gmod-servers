...
hook.Add(
    "atlaschat.DatabaseConnected",
    "1",
    function(remote, firstTime)
        if firstTime then
            atlaschat.sql.Query(
                "CREATE TABLE IF NOT EXISTS atlaschat_players(id INTEGER PRIMARY KEY " ..
                    (remote and "AUTO_INCREMENT" or "") .. ", steamID TEXT, title TEXT)"
            )
            atlaschat.sql.Query(
                "CREATE TABLE IF NOT EXISTS atlaschat_ranks(id INTEGER PRIMARY KEY " ..
                    (remote and "AUTO_INCREMENT" or "") .. ", usergroup TEXT, icon TEXT, tag TEXT)"
            )
            atlaschat.sql.Query(
                "CREATE TABLE IF NOT EXISTS atlaschat_restrictions(id INTEGER PRIMARY KEY " ..
                    (remote and "AUTO_INCREMENT" or "") .. ", expression TEXT, usergroups TEXT)"
            )

            atlaschat.sql.Query(
                "SELECT * FROM atlaschat_ranks",
                function(data, query)
                    if data then
                        for i = 1, #data do
                            local info = data[i]
            ...
            atlaschat.sql.Query(
                "SELECT * FROM atlaschat_restrictions",
                function(data, query)
                    if data then
                        for i = 1, #data do
                            local info = data[i]
...
net.Receive(
    ...
    function(bits, player)
        ...
        if not loaded then
            ...
            atlaschat.sql.Query(
                "SELECT * FROM atlaschat_players WHERE steamID=" .. steamID,
                function(data, query)
...
net.Receive(
    "atlaschat.stplttl",
    function(bits, player)
        if player:IsSuperAdmin() or string.match(player:GetUserGroup(), "vip") then
            ...
            if IsValid(target) then
                ...
                atlaschat.sql.Query(
                    "UPDATE atlaschat_players SET title=" .. sql.SQLStr(title) .. " WHERE steamID=" .. steamID
                )
            end
        end
    end
    ...
    function(bits, player)
        ...
        if isAdmin then
            ...
            if atlaschat.ranks[userGroup] ~= nil then
                ...
                atlaschat.sql.Query(
                    "INSERT INTO atlaschat_ranks(id, usergroup, icon, tag) VALUES(NULL, " ..
                        sql.SQLStr(userGroup) .. ", " .. sql.SQLStr("icon16/user.png") .. ", " .. sql.SQLStr("") .. ")"
                )
    ...
    function(bits, player)
        ...
        if isAdmin then
            ...
            if atlaschat.ranks[userGroup] then
                ...
                atlaschat.sql.Query(
                    "UPDATE atlaschat_ranks SET icon = " ..
                        sql.SQLStr(icon) ..
                            ", tag = " .. sql.SQLStr(tag) .. " WHERE usergroup = " .. sql.SQLStr(userGroup)
                )
                ...
                atlaschat.Notify(
                    ":information: Successfully changed the icon/tag of usergroup '" ..
                        userGroup .. "' to '" .. icon .. "  -  " .. tag .. "'!",
                    player
                )
            else
    ...
    function(bits, player)
        ...
        if isAdmin then
            ...
            if atlaschat.sql.IsRemote() then
                atlaschat.sql.Query(
                    "SELECT id FROM atlaschat_restrictions WHERE expression = " .. sql.SQLStr(expression),
                    function(data, query)
                        if #data > 0 then
                            atlaschat.sql.Query(
                                "UPDATE atlaschat_restrictions SET usergroups = " ..
                                    sql.SQLStr(info) .. " WHERE expression = " .. sql.SQLStr(expression)
                            )
                        else
                            atlaschat.sql.Query(
                                "INSERT INTO atlaschat_restrictions(id, expression, usergroups) VALUES(NULL, " ..
                                    sql.SQLStr(expression) .. ", " .. sql.SQLStr(info) .. ")"
                            )
                        end
                    end
                )
                ...
                atlaschat.sql.Query(
                    "INSERT OR REPLACE INTO atlaschat_restrictions(id, expression, usergroups) VALUES((SELECT id FROM atlaschat_restrictions WHERE expression = " ..
                        sql.SQLStr(expression) .. "), " .. sql.SQLStr(expression) .. ", " .. sql.SQLStr(info) .. ")"
                )
            end
        end
    end
...
net.Receive(
    "atlaschat.txpm",
    function(bits, player)
        if player.nextMessageAtlas > CurTime() then
            return
        end
...
net.Receive(
    ...
    function(bits, player)
        ...
        if target ~= "" then
            ...
            if IsValid(target) then
                if target ~= player and not player:IsAdmin() then
                    return
                end
                ...
                if target == player then
                    atlaschat.Notify(
                        "Successfully reset your atlaschat configuration! Close and open your chatbox to apply.",
                        target
                    )
                else
                    atlaschat.Notify(
                        player:Nick() ..
                            " has reset your atlaschat configuration! Close and open your chatbox to apply.",
                        target
                    )
                    atlaschat.Notify(
                        "You have reset " ..
                            target:Nick() .. "'s atlaschat configuration! Close and open your chatbox to apply.",
                        player
                    )
                end
            end
            ...
            if player:IsAdmin() then
                ...
                atlaschat.Notify(
                    player:Nick() ..
                        " has reset everyone's atlaschat configuration! Close and open your chatbox to apply."
                )
            end
        end
    end
