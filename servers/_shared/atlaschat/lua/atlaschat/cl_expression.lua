...
function expression:Execute(base, font, text)
    ...
    if not ok then
        ...
        timer.Simple(
            0.05,
            function()
                chat.AddText(':exclamation: The font "' .. eugh .. '" is invalid!')
            end
        )
...
local expression =
    atlaschat.expression.New("<cflash%s*(c?=?%d-,-%d-,-%d-)%s*(c?=?%d-,-%d-,-%d-)>(.-)</cflash>", "cflash")
...
function expression:GetExample(base)
    return "<icon 32, 32>icon16/user.png</icon>", self:Execute(base, 32, 32, "icon16/user.png")
end
...
local expression = atlaschat.expression.New("<s>(.-)</s>", "steamemote")

expression.cleanName = "<s> </s>"
...
function expression:GetExample(base)
    return "<s>emofdr</s>", self:Execute(base, "emofdr")
end
