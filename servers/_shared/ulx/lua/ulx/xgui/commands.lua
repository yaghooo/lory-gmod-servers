...
function cmds.refreshPlist(arg)
	if not arg then
		arg = ULib.cmds.translatedCmds[cmds.selcmd].args[2]
	end
	if not arg or (arg.type ~= ULib.cmds.PlayersArg and arg.type ~= ULib.cmds.PlayerArg) then
		return
	end
	...
	for k, Line in pairs(cmds.plist.Lines) do
		if (Line:IsLineSelected()) then
			table.insert(lastplys, Line:GetColumnText(1))
		end
	end
	...
	if not cmds.plist:GetSelectedLine() then
		if not xlib.animRunning then
			...
			if cmds.permissionChanged then
				xlib.addToAnimQueue(
					function()
						if cmds.argslist.open and cmds.plist.open then
							cmds.argslist:Close()
						end
					end
				)
...
function cmds.buildArgsList(cmd)
	...
	local argnum = 0
	local zpos = 0
	local expectingplayers =
		cmd.args[2] and ((cmd.args[2].type == ULib.cmds.PlayersArg) or (cmd.args[2].type == ULib.cmds.PlayerArg)) or false
				...
				if arg.invisible ~= true then
					...
					if curitem.repeat_min then --This command repeats!
						...
						panel.addbutton.DoClick = function(self)
							...
							if parent.arg.repeat_max and #choices >= parent.arg.repeat_max then
						...
						panel.removebutton.DoClick = function(self)
							...
							if #choices <= parent.arg.repeat_min then
								self:SetDisabled(true)
							end
						end
	...
	for cmd, data in pairs(ULib.cmds.translatedCmds) do
		...
		if opposite ~= cmd and (LocalPlayer():query(data.cmd) or (opposite and LocalPlayer():query(opposite))) then
			...
			if catname == nil or catname == "" then
				catname = "_Uncategorized"
			end
			if not cmds.cmd_contents[catname] then
				--Make a new category
				cmds.cmd_contents[catname] = xlib.makelistview {headerheight = 0, multiselect = false, h = 136}
				cmds.cmd_contents[catname].OnRowSelected = function(self, LineID)
					cmds.setselected(self, LineID)
				end
				cmds.cmd_contents[catname]:AddColumn("")
				local cat =
					xlib.makecat {label = catname, contents = cmds.cmd_contents[catname], expanded = false, parent = xgui.null}
...
function cmds.argslist:openAnim(cmd, secondary)
	xlib.addToAnimQueue(
		function()
			cmds.argslist.secondaryPos = secondary
		end
	)
	...
	if secondary then
		xlib.addToAnimQueue(
			"pnlSlide",
			{panel = self.scroll, startx = -170, starty = 0, endx = 0, endy = 0, setvisible = true}
		)
	else
		xlib.addToAnimQueue(
			"pnlSlide",
			{panel = self.scroll, startx = 80, starty = 0, endx = 255, endy = 0, setvisible = true}
		)
	end
end
...
function cmds.argslist:closeAnim(secondary)
	if secondary then
		xlib.addToAnimQueue(
			"pnlSlide",
			{panel = self.scroll, startx = 0, starty = 0, endx = -170, endy = 0, setvisible = false}
		)
	else
		xlib.addToAnimQueue(
			"pnlSlide",
			{panel = self.scroll, startx = 255, starty = 0, endx = 80, endy = 0, setvisible = false}
		)
	end
	xlib.addToAnimQueue(
		function()
