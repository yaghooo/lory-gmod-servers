...
function groups.init()
	ULib.ucl.registerAccess(
		"xgui_managegroups",
		"superadmin",
		"Allows managing of groups, users, and access strings via the groups tab in XGUI.",
		"XGUI"
	)
			...
			while group do
				if group == args[1] or args[1] == args[2] then
					ULib.clientRPC(
						ply,
						"Derma_Message",
						"Cannot set inheritance! You cannot inherit from something you're inheriting to!",
						"XGUI NOTICE"
					)
					return
				end
	...
	ULib.ucl.addUser = function(id, allows, denies, group)
		...
		if affectedply then
			groups.resetAllPlayerValues(affectedply)
		end
			...
			for i, groupname in ipairs(teamdata.groups) do
				if group == groupname then --Found the previous team the group belonged to, remove it now!
					...
					for modifier, _ in pairs(teamdata) do
						if
							modifier ~= "order" and modifier ~= "index" and modifier ~= "groups" and modifier ~= "name" and
								modifier ~= "color"
						 then
					...
					if modifier == "color" then
						...
						if value ~= "" then
							...
							if type(def) == "table" then
								if def[2] and value < def[2] then
									value = def[2]
								end
								if def[3] and value > def[3] then
									value = def[3]
								end
							end
							v[modifier] = value
	...
	function groups.refreshTeams()
		if not ulx.uteamEnabled() then
			return
		end --Do not perform any of the following code if UTeam is disabled.
	...
	function groups.resetPlayerValue(ply, values)
		for _, modifier in ipairs(values) do
			...
			if type(defaultvalue) == "table" then
				defaultvalue = xgui.teamDefaults[modifier][1]
			end
	...
	function groups.resetTeamValue(group, values, teamIsUnassigned)
		for _, ply in ipairs(player.GetAll()) do
			if ply:GetUserGroup() == group then
				...
				if teamIsUnassigned then
					ply:SetTeam(1001)
				end --Force the player to the unassigned team
			end
		end
	end
