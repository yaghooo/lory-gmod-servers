...
function bans.init()
	ULib.ucl.registerAccess(
		"xgui_managebans",
		"superadmin",
		"Allows addition, removal, and viewing of bans in XGUI.",
		"XGUI"
	)
	...
	xgui.addDataType(
		"bans",
		function()
			return {count = table.Count(ULib.bans)}
		end,
		"xgui_managebans",
		30,
		20
		...
		if sortType == 1 then
			...
			if next(xgui.bansbyname) == nil then
				...
				table.sort(
					xgui.bansbyname,
					function(a, b)
						return (a[2] or "\255" .. a[1]) < (b[2] or "\255" .. b[1])
					end
				)
			end
		...
		elseif sortType == 6 then
			...
			if next(xgui.bansbybanlength) == nil then
				...
				table.sort(
					xgui.bansbybanlength,
					function(a, b)
						return (a[2] or math.huge) < (b[2] or math.huge)
					end
				)
			end
	...
	function bans.sendBansToUser(ply, args)
		...
		if not ULib.ucl.query(ply, "xgui_managebans") then
			return
		end
					...
					if
						not (filterString and
							not (steamID and string.find(string.lower(steamID), filterString) or
								...
								bandata.modified_admin and string.find(string.lower(bandata.modified_admin), filterString)))
					 then
						...
						if #bansToSend < 17 and currentEntry >= firstEntry then
							...
							if noFilter and #bansToSend >= 17 then
								break
							end -- If there is a filter, then don't stop the loop so we can get a "result" count.
						end
