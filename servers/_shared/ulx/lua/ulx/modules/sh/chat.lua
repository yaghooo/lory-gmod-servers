...
end -- Give operators access to see asays echoes by default
...
local function doAdvert(group, id)
	...
	ULib.queueFunctionCall(
		function()
			...
			timer.Create(
				"ULXAdvert" .. type(group) .. group,
				adverts[group][nextid].rpt,
				1,
				function()
					doAdvert(group, nextid)
				end
			)
		end
	)
...
if SERVER then
	...
	local function meCheck(ply, strText, bTeam)
		...
		if ply.gimp or meChatEnabled == 0 or (meChatEnabled ~= 2 and GAMEMODE.Name ~= "Sandbox") then
			return
		end -- Don't mess
...
if SERVER then
	ulx.convar(
		"meChatEnabled",
		"1",
		"Allow players to use '/me' in chat. 0 = Disabled, 1 = Sandbox only (Default), 2 = Enabled",
		ULib.ACCESS_ADMIN
	)
