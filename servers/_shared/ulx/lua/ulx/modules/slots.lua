...
ulx.convar(
	"rslotsVisible",
	"1",
	" - Sets whether slots are visible. See config for more information.",
	ULib.ACCESS_ADMIN
)
...
function calcSlots(disconnect)
	...
	if mode == 3 then
		return 1
	end -- Only one slot on this mode
	...
	if mode == 1 then
		...
		if disconnect then
			admins = admins - 1
		end -- Otherwise we're counting the disconnecting admin
		if admins < 0 then
			admins = 0
		end -- Just to be safe!
		...
		if rslots < 0 then
			rslots = 0
		end -- If we have more admins right now then slots for them, we don't want to return a negative number.
		return rslots
	end
