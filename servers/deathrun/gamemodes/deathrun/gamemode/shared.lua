...
function player.GetAllPlaying()
    ...
    for k, ply in ipairs(player.GetAll()) do
        if ply and not ply:ShouldStaySpectating() then
...
hook.Add("SetupMove", "DeathrunDisableSpectatorSpacebar", function(ply, mv, cmd)
    ...
    if ply:Alive() and ROUND:GetCurrent() == ROUND_PREP then
        --mv:SetButtons( bit.band( mv:GetButtons(), bit.bnot( IN_JUMP ) ) )
        local block = hook.Call("DeathrunPreventPreptimeMovement") or true

        -- block movement for runners
        if block == true and ply:Team() == TEAM_RUNNER then
            mv:SetSideSpeed(0)
            mv:SetUpSpeed(0)
...
local mc, bn, ba, gf = math.Clamp, bit.bnot, bit.band, {}
