...
function MV:GetGoodMaps()
    ...
    for k, map in ipairs(mapfiles) do
        for _, filter in ipairs(MV.Filter) do
            ...
            if mapname == filter and not table.HasValue(goodmaps, map) then
...
concommand.Add("mapvote_begin_mapvote", function(ply, cmd, args)
    if DR:CanAccessCommand(ply, cmd) and not hook.Call("DeathrunStartMapvote", nil, ROUND:GetRoundsPlayed()) then
        MV:BeginMapVote()
    end
end)
...
function MV:CheckRTV(suppress)
    ...
    if not suppress and MV.LoadTime + 60 > CurTime() then
