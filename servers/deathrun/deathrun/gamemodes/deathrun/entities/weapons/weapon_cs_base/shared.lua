...
-- stole the code from my drug addon lol - arizard
function SWEP:CalculateFalloff(drunkhigh, dt)
...
function SWEP:BulletLaserCallback(ply, tr, dmginfo)
...
function SWEP:CSShootBullet(dmg, recoil, numbul, cone)
    ...
    bullet.Callback = function(ply, tr, dmginfo)
        if self.LaserBeams then
            self:BulletLaserCallback(ply, tr, dmginfo)
        end
    end
...
if CLIENT then
    ...
    function SWEP:AdjustMouseSensitivity()
        if self:GetIronsights() then
            if self.Scope == true then return self.ScopedFOV / 70 end
        end
    end
end
