...
local function getInitialSpeeds(ply) -- fetch players' initial walk and run speeds, it's different for each gamemode
	timer.Simple(
		0.5,
		function()
			-- for some reason this prints the run speed different than it actually is if i dont add the timer...
			if ply:IsValid() then
...
if (CLIENT) then
	...
	cvars.AddChangeCallback(
		"eastereggs_enable",
		function(cvar, oldValue, newValue)
