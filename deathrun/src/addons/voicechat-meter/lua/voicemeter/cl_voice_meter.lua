...
function VoiceChat.StartVoice(ply)
	if not ply:IsValid() or not ply.Team then
		return
	end
	...
	NameBar:SetSize(W, H)
	local StartPos =
		(VoiceChatMeter.SlideOut and ((VoiceChatMeter.PosX < .5 and -W) or ScrW())) or
		(ScrW() * VoiceChatMeter.PosX - (VoiceChatMeter.Align == 1 and 0 or W))
	NameBar:SetPos(StartPos, ScrH() * VoiceChatMeter.PosY + ToAdd)
	if VoiceChatMeter.SlideOut then
		NameBar:MoveTo(
			(ScrW() * VoiceChatMeter.PosX - (VoiceChatMeter.Align == 1 and 0 or W)),
			ScrH() * VoiceChatMeter.PosY + ToAdd,
			VoiceChatMeter.SlideTime
		)
	end
	...
	function NameBar:Think()
		...
		if self.Next and curTime - self.Next < 0.1 then
			return false
		end
...
hook.Add(
	...
	"RemoveOldVoiceChat",
	function(elem)
		if elem == "CHudVoiceStatus" or elem == "CHudVoiceSelfStatus" then
			return false
		end
	end
)
