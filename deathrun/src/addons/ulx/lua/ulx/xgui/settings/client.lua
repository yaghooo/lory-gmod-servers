...
client.catList.Columns[1].DoClick = function()
end
...
genpnl.pickupplayers =
	xlib.makecheckbox {
	x = 10,
	y = 10,
	w = 150,
	label = "Enable picking up players with physgun (for yourself)",
	convar = "cl_pickupplayers",
	parent = genpnl
}
...
xlib.makeslider {
		...
		label = "<--->",
		max = 2,
		value = xgui.settings.animTime,
		decimal = 2,
		parent = xguipnl
	}.OnValueChanged = function(self, val)
	local testval = math.Clamp(tonumber(val), 0, 2)
	if testval ~= tonumber(val) then
		self:SetValue(testval)
	end
...
xlib.makecheckbox {
		...
		y = 97,
		w = 150,
		label = "Show Startup Messages",
		value = xgui.settings.showLoadMsgs,
		parent = xguipnl
	}.OnChange = function(self, bVal)
	xgui.settings.showLoadMsgs = bVal
end
xlib.makelabel {x = 10, y = 120, label = "Infobar color:", parent = xguipnl}

xlib.makecolorpicker {
		x = 10,
		y = 135,
		color = xgui.settings.infoColor,
		addalpha = true,
		alphamodetwo = true,
		parent = xguipnl
	}.OnChangeImmediate = function(self, color)
...
xguipnl.updateMainOrder = function()
	...
	for i, v in ipairs(xgui.settings.moduleOrder) do
		...
		if found then
			...
			if v == selected then
				xguipnl.mainorder:SelectItem(l)
			end
		else
...
xguipnl.updateSettingOrder = function()
	...
	for i, v in ipairs(xgui.settings.settingOrder) do
		...
		if found then
			...
			if v == selected then
				xguipnl.settingorder:SelectItem(l)
			end
		else
...
xguipnl.b7.DoClick = function(self)
	xguipnl.updatePos(7)
end
xguipnl.b8 =
	xlib.makebutton {
...
xguipnl.b4.DoClick = function(self)
	xguipnl.updatePos(4)
end
xguipnl.b5 =
	xlib.makebutton {
...
xguipnl.b5.DoClick = function(self)
	xguipnl.updatePos(5)
end
xguipnl.b6 =
	xlib.makebutton {
...
xguipnl.xwang =
	...
	min = -1000,
	max = 1000,
	value = xgui.settings.xguipos.xoff,
	decimal = 0,
	parent = xguipnl
}
...
xguipnl.ywang =
	...
	min = -1000,
	max = 1000,
	value = xgui.settings.xguipos.yoff,
	decimal = 0,
	parent = xguipnl
}
...
xguipnl.inAnim =
	...
	y = 245,
	w = 150,
	choices = {"Fade In", "Slide From Top", "Slide From Left", "Slide From Bottom", "Slide From Right"},
	parent = xguipnl
}
...
xguipnl.outAnim =
	...
	y = 270,
	w = 150,
	choices = {"Fade Out", "Slide To Top", "Slide To Left", "Slide To Bottom", "Slide To Right"},
	parent = xguipnl
}
