...
function Player:PS_PlayerInitialSpawn()
    ...
    if PS.Config.PointsOverTime then
        timer.Create(
            "PS_PointsOverTime_" .. self:UniqueID(),
            PS.Config.PointsOverTimeDelay * 60,
            0,
            function()
                if not IsValid(self) or self.Spectating then
                    return
                end
...
function Player:PS_GiveItem(item_id)
    ...
    local defaultProps = {Modifiers = {}, Equipped = false}
...
function Player:PS_BuyItem(item_id)
    ...
    if not self:PS_HasPoints(points) or not self:PS_CanEquipItem(ITEM) then
        return false
    end
...
function Player:PS_EquipItem(item_id)
    ...
    if not ITEM or not self:PS_HasItem(item_id) or not self:PS_CanEquipItem(ITEM) then
        return false
    end
            ...
            self:PS_Notify(
                "Somente " ..
                    CATEGORY.AllowedEquipped ..
                        " item" ..
                            (CATEGORY.AllowedEquipped == 1 and "" or "s") .. " podem ser equipados desta categoria!"
            )
            return false
        end
    end
...
function Player:PS_ModifyItem(item_id, modifications)
    if not PS.Items[item_id] or not self:PS_HasItem(item_id) or not type(modifications) == "table" then
        return false
    end
...
function Player:PS_CanEquipItem(ITEM)
    ...
    if ITEM.AllowedUserGroups and #ITEM.AllowedUserGroups > 0 then
        if not table.HasValue(ITEM.AllowedUserGroups, self:GetUserGroup()) then
    ...
    if CATEGORY.AllowedUserGroups and #CATEGORY.AllowedUserGroups > 0 then
        if not table.HasValue(CATEGORY.AllowedUserGroups, self:GetUserGroup()) then
            self:PS_Notify("Você precisa ser vip para usar isso!")
            return false
        end
    end
...
function Player:PS_RemoveClientsideModel(item_id)
    ...
    if not PS.ClientsideModels[self] or not PS.ClientsideModels[self][item_id] then
        return false
    end
...
function Player:Log(text, ...)
    ulx.logString(
        string.format([[[Pointshop] Player %s with steamid %s did: ]] .. text, self:Nick(), self:SteamID(), ...)
    )
end
