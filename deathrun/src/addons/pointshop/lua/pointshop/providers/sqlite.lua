local function QueryWithInsertOrIgnore(ply, query, ...)
    ...
    ply:Log("Handled the query '%s'", query)
...
sql.Query(
    "CREATE TABLE IF NOT EXISTS `pointshop_data` ( `sid64` STRING, `points` REAL, `items` STRING, PRIMARY KEY(sid64) )"
)

function PROVIDER:GetData(ply, callback)
    local query = [[SELECT points, items FROM `pointshop_data` WHERE sid64 = '%s']]
    ...
    if data and #data > 0 then
        ...
        local items = row.items and util.JSONToTable(row.items) or {}
