...
hook.Add(
    ...
    function(ply)
        ...
        if
            ply == LocalPlayer() and GetViewEntity():GetClass() == "player" and
                (GetConVar("thirdperson") and GetConVar("thirdperson"):GetInt() == 0)
         then
            return
        end
        if not PS.ClientsideModels[ply] then
            return
        end
