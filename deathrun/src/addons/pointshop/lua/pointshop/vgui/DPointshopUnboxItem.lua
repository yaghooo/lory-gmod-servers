...
function PANEL:SetData(wonItem, hasItem, readonly)
    ...
    if not readonly then
        ...
        sell.SellText =
            "Vender: " .. PS.Config.CalculateSellPrice(LocalPlayer(), wonItem) .. " " .. PS.Config.PointsName
        function sell:Paint(w, h)
            ...
            draw.SimpleText(
                hasItem and "Você já possui! Vendido automaticamente." or self.SellText,
                "PS_CatName",
                w / 2,
                h / 2,
                hasItem and Color(20, 20, 20, 180) or ColorAlpha(color_white, 180),
                TEXT_ALIGN_CENTER,
                TEXT_ALIGN_CENTER
            )
        end
