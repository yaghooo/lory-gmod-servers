...
DR:AddCustomHUD(
    2,
    function(x, y)
        DR:DrawPlayerHUDSass(x, y)
    end,
    function(x, y)
        DR:DrawPlayerHUDAmmoSass(x, y)
    end
)
--classichud
DR:AddCustomHUD(
...
function GM:HUDPaint()
    ...
    if RoundEndData.Active then -- check if it's stalemate, and don't do the thing, zhu li!
        DR:DrawWinners(
            ...
            ScrW() / 2 - 628 / 2,
            24,
            RoundEndData.winteam == 1 and true or false
        )
...
function DR:DrawTargetID()
    ...
    local tidText =
        DR.TargetIDName ..
        (IsValid(DR.TargetIDPlayer) and " - " .. tostring(math.Clamp(DR.TargetIDPlayer:Health(), 0, 100)) .. "%" or "")
    ...
    deathrunShadowTextSimple(
        ...
        x,
        y,
        Color(255, 255, 255, DR.TargetIDColor.a * 0.2),
        TEXT_ALIGN_CENTER,
        TEXT_ALIGN_CENTER
    )
...
function DR:DrawPlayerHUD(x, y)
    ...
    local shouldDrawTime =
        ROUND:GetCurrent() == 5 and ply == LocalPlayer() and ply:Team() == TEAM_RUNNER and HudTheme:GetInt() == 1
    ...
    deathrunShadowTextSimple(
        string.upper(RoundNames[ROUND:GetCurrent()] or "TIME LEFT"),
        "deathrun_hud_Small",
        dx + 4,
        dy + 16 / 2,
        otcol,
    ...
    deathrunShadowTextSimple(
        string.ToMinutesSeconds(math.Clamp(ROUND:GetTimer(), 0, 99999)),
        "deathrun_hud_Small",
        dx + 228 - 4,
        dy + 16 / 2,
        otcol,
    ...
    deathrunShadowTextSimple(
        tostring(curvel) ..
            ((ply.AutoJumpEnabled == true and GetConVar("deathrun_allow_autojump"):GetBool() == true) and " AUTO" or ""),
        "deathrun_hud_Large",
        dx + 32 + 4 + 4,
        dy + 32 / 2 - 1,
        DR.Colors.Text.Clouds,
...
function DR:DrawPlayerHUDAmmo(x, y)
    ...
    if wepdata.HoldType == "melee" or wepdata.HoldType == "knife" then
        return
    end
    ...
    if IsValid(wep) then
        ...
        if wepdata.ShouldDrawHUD then
            deathrunShadowTextSimple(
                tostring(wepdata.Clip1) .. " +" .. tostring(wepdata.Remaining1),
                "deathrun_hud_Large",
                dx + 32 + 192,
                dy + 32 / 2 - 1,
                DR.Colors.Text.Clouds,
...
function DR:DrawWinners(winteam, tbl_mvps, x, y, stalemate)
    ...
    deathrunShadowTextSimple(
        stalemate == false and string.upper(team.GetName(winteam) .. " win the round!") or "STALEMATE!",
        "deathrun_hud_Xlarge",
        x + w / 2,
        y + h / 2,
        DR.Colors.Text.Clouds,
...
function DR:DrawPlayerHUDSass(x, y)
    ...
    deathrunShadowTextSimple(
        teamtext .. " - " .. string.ToMinutesSeconds(math.Clamp(ROUND:GetTimer(), 0, 99999)),
        "sassSmall",
        x + 8,
        y + h / 2 + 24,
        Color(255, 255, 255, 255),
...
function DR:DrawPlayerHUDClassic(x, y)
    ...
    draw.RoundedBox(
        ...
        (hw - 8) * hpfrac,
        hh - 8,
        Color(80, 180, 60, 255 * amul)
    )

    deathrunShadowText(
        tostring(curhp > 999 and "dafuq" or math.max(curhp, 0)),
        "Deathrun_SmoothBig",
        x + w / 2 - hw / 2 + 5,
        y + h - hh,
        Color(255, 255, 255),
        ...
        if obj then
            ...
            if obj.hp > 0 then
                ...
                deathrunShadowTextSimple(
                    ...
                    x,
                    y + dy + sh * 16,
                    modecol[obj.mode] or Color(0, 0, 0),
                    TEXT_ALIGN_CENTER,
                    TEXT_ALIGN_BOTTOM,
                    1
                )
