...
hook.Add(
    "PlayerDisconnected",
    "DeathrunPlayerDisconnectMessage",
    function(ply)
...
local dropWeaponsOnDeath =
    CreateConVar("deathrun_drop_weapons_on_death", 1, defaultFlags, "Should players drop weapons on death?")

hook.Add(
    "PlayerSpawn",
    "DeathrunSetPlayerModels",
    function(ply)
        ...
        if ply:Team() == TEAM_DEATH then
            ...
            if string.sub(mdl, -4, -1) == ".mdl" then
                ...
                print(
                    "The default death model is not a valid .mdl file ('" ..
                        mdl .. "'). Please change the deathrun_death_model ConVar."
                )
            end
...
hook.Add(
    "AcceptInput",
    "DeathrunKillers",
    function(ent, input, activator, caller)
    ...
    timer.Simple(
        ...
        function()
            if not IsValid(ply) then
                return
            end -- incase they die and disconnect, prevents console errors.
...
function GM:CanPlayerSuicide(ply)
    ...
    if (not ply:Alive()) or (ply:GetSpectate()) then
        return false
    end -- don't let dead players or spectators suicide
    if ply:Team() == TEAM_DEATH then
        return false
    end -- never allow suicide on death team
    if ply:Team() == TEAM_GHOST then
        return false
    end -- never allow suicide on ghost team
    if ROUND:GetCurrent() == ROUND_PREP then
        return false
    end -- players cannot suicide during round prep time
...
function GM:PlayerCanHearPlayersVoice(listener, talker)
    ...
    if table.HasValue(listener.mutelist, talker:SteamID()) then
        ...
        if alltalk:GetBool() == false then
            if talker:GetSpectate() == true and listener:GetSpectate() == false then
                return false
            end
            if talker:Alive() == false and listener:Alive() == true then
                return false
            end
...
hook.Add(
    ...
    "StopWeaponAbuseAustraliaSaysNo",
    function(ply, wep)
        if ply:Team() == TEAM_GHOST then
            return false
        end
        ...
        if wep.Slot == 1 and slot1 > 0 then
            return false
        end
        if wep.Slot == 3 and slot3 > 0 then
            return false
        end
...
timer.Create(
    ...
    function()
        for k, ply in ipairs(player.GetAllPlaying()) do -- don't kick afk spectators or bots
            ...
            if
                DR:CheckIdleTime(ply) > IdleTimer:GetInt() and ply:SteamID() ~= "BOT" and (not ply:IsAdmin()) and
...
DR.BarredPlayers =
    util.JSONToTable(file.Read(deathbarred_path, "DATA")) or
    {["STEAMID_EXAMPLE"] = {rounds = 3, lastpunish = os.time()}}
