...
tsaycolor:addParam {
	...
	hint = "color",
	completes = ulx_tsay_color_table,
	ULib.cmds.restrictToCompletes
} -- only allows values in that table
...
if SERVER then
	ULib.ucl.registerAccess(seesasayAccess, ULib.ACCESS_SUPERADMIN, "Ability to see 'ulx sasay'", "Other")
end
...
csaycolor:addParam {
	type = ULib.cmds.StringArg,
	hint = "color",
	completes = ulx_csay_color_table,
	ULib.cmds.restrictToCompletes
} -- only allows values in that table
...
notifications:addParam {
	type = ULib.cmds.StringArg,
	hint = "type",
	completes = notification_types_table,
	ULib.cmds.restrictToCompletes
}
