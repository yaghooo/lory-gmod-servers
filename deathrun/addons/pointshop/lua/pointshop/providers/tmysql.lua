...
local function MySQL_CreateDatabase()
    MySQL_Query(
        "CREATE TABLE IF NOT EXISTS pointshop_data ( uniqueid varchar( 30 ) NOT NULL PRIMARY KEY, points int( 32 ) NOT NULL, items text NOT NULL )"
    )
end
...
function PROVIDER:GetData(ply, callback)
    MySQL_Query(
        string.format("SELECT * FROM pointshop_data WHERE uniqueid = '%s'", ply:UniqueID()),
        function(data)
...
function PROVIDER:SetPoints(ply, points)
    MySQL_Query(
        string.format(
            "INSERT INTO pointshop_data ( uniqueid, points, items ) VALUES ( '%s', '%s', '[]' ) ON DUPLICATE KEY UPDATE points = VALUES ( points )",
            ply:UniqueID(),
            points
        )
    )
...
function PROVIDER:GivePoints(ply, points)
    MySQL_Query(
        string.format(
            "INSERT INTO pointshop_data ( uniqueid, points, items ) VALUES ( '%s', '%s', '[]' ) ON DUPLICATE KEY UPDATE points = points + VALUES ( points )",
            ply:UniqueID(),
            points
        )
    )
...
function PROVIDER:GiveItem(ply, item_id, data)
    ...
    MySQL_Query(
        string.format(
            "INSERT INTO pointshop_data ( uniqueid, points, items ) VALUES ( '%s', '0', '%s' ) ON DUPLICATE KEY UPDATE items = VALUES ( items )",
            ply:UniqueID(),
...
function PROVIDER:SetData(ply, points, items)
    MySQL_Query(
        string.format(
            "INSERT INTO pointshop_data ( uniqueid, points, items ) VALUES ( '%s', '%s', '%s' ) ON DUPLICATE KEY UPDATE points = VALUES ( points ), items = VALUES( items )",
            ply:UniqueID(),
            points,
            items
        )
