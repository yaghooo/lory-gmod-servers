...
net.Receive(
    ...
    function(length, ply)
        ...
        if not PS.Config.CanPlayersGivePoints then
            return
        end
        if not points or points == 0 then
            return
        end
        if not other or not IsValid(other) or not other:IsPlayer() then
            return
        end
...
net.Receive(
    ...
    function(length, ply)
        ...
        if not PS.Config.CanPlayersGiveItems then
            return
        end
        if not item_id then
            return
...
net.Receive(
    ...
    function(length, ply)
        ...
        if not PS.Config.AdminCanAccessAdminTab and not PS.Config.SuperAdminCanAccessAdminTab then
            return
        end
...
net.Receive(
    ...
    function(length, ply)
        ...
        if
            (admin_allowed or super_admin_allowed) and other and item_id and PS.Items[item_id] and IsValid(other) and
                other:IsPlayer() and
                not other:PS_HasItem(item_id)
         then
            other:PS_GiveItem(item_id)
...
hook.Add(
    ...
    function()
        ...
        for _, v in pairs(player.GetAll()) do
            if not IsValid(v) or v.Spectating then
                return
            end
...
function PS:GivePlayerItem(ply, item_id)
    self.DataProvider:GiveItem(ply, item_id)
end
