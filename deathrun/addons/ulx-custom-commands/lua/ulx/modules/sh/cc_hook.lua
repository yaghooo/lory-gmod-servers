...
function ulx.addhook(calling_ply, hooktype, hookid, args, string, should_print)
	if not should_print then
		RunString(
			"function " ..
				hookid ..
					"( " .. args .. " ) " .. string .. ' end hook.Add( "' .. hooktype .. '", "' .. hookid .. '", ' .. hookid .. " )"
		)
		...
		ulx.fancyLogAdmin(
			calling_ply,
			true,
			"#A hooked type #s with id #s with args #s which runs #s",
			hooktype,
			hookid,
			args,
			string
...
function ulx.removehookcl(calling_ply, target_ply, hooktype, hookid)
	...
	ulx.fancyLogAdmin(
		calling_ply,
		true,
		"#A removed hook type #s with identifier #s from #T",
		hooktype,
		hookid,
		target_ply
	)
...
local gethooktable = ulx.command("Utility", "ulx gethooktable", ulx.gethooktable, "!gethooktable", true)
