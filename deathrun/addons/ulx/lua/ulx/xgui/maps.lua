...
maps.svote =
	xlib.makebutton {x = 185, y = 270, w = 192, h = 20, label = "Server-wide vote of selected map(s)", parent = maps}
...
maps.changemap =
	...
	h = 20,
	disabled = true,
	label = "Force changelevel to this map",
	parent = maps
}
maps.changemap.DoClick = function()
	if maps.curmap:GetValue() ~= "No Map Selected" then
		Derma_Query(
			'Are you sure you would like to change the level to "' .. maps.curmap:GetValue() .. '"?',
			"XGUI WARNING",
			"Change Level",
			function()
				RunConsoleCommand(
...
maps.nextlevel =
	...
	w = 180,
	h = 20,
	repconvar = "rep_nextlevel",
	convarblanklabel = "<not specified>",
	parent = maps
}
...
function maps.updateVoteMaps()
	...
	for k, Line in pairs(maps.list.Lines) do
		if (Line:IsLineSelected()) then
			table.insert(lastselected, Line:GetColumnText(1))
		end
	end
...
function maps.updateGamemodes()
	...
	local restrictions = {}
	ULib.cmds.StringArg.processRestrictions(
		restrictions,
		ULib.cmds.translatedCmds["ulx map"].args[3],
