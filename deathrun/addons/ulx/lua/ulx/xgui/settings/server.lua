...
xlib.makecombobox {
	...
	isNumberConvar = true,
	choices = {"Team near you", "Team only", "Everyone near you", "Everyone"},
	parent = server
}
...
server.catList.Columns[1].DoClick = function()
end
server.catList.OnRowSelected = function(self, LineID, Line)
	...
	if nPanel ~= server.curPanel then
		...
		if nPanel.onOpen then
			xlib.addToAnimQueue(nPanel.onOpen)
		end --If the panel has it, call a function when it's opened
		xlib.addToAnimQueue(
			"pnlSlide",
			{panel = server.panel, startx = -285, starty = 0, endx = 5, endy = 0, setvisible = true}
		)
	else
		--Close
		server.curPanel = nil
...
function adverts.getNodePos(node)
	if type(node.group) == "number" then
		return 1
	end
	local parentchildren = node:GetParentNode().ChildNodes:GetChildren()
	local output = 0
	if parentchildren[#parentchildren] == node then
		output = 1
	end
	if parentchildren[1] == node then
		output = output + 2
	end
	return output
end
adverts.tree.DoRightClick = function(self, node)
	...
	if not node.data then
		menu:AddOption(
			"Rename Group...",
			function()
				adverts.RenameAdvert(node:GetText())
			end
		)
	end
	menu:AddOption(
...
adverts.seloffset = 0
adverts.message =
	xlib.maketextbox {x = 125, w = 150, h = 20, text = "Enter a message...", parent = adverts, selectall = true}
...
adverts.time =
	...
	min = 1,
	max = 1000,
	tooltip = "Time in seconds till the advert is shown/repeated.",
	parent = adverts
}
adverts.group =
	xlib.makecombobox {
	...
	enableinput = true,
	parent = adverts,
	tooltip = "Select or create a new advert group."
}
...
adverts.display =
	...
	max = 60,
	value = 10,
	tooltip = "The time in seconds the CSay advert is displayed"
}
...
xlib.makebutton {x = 200, y = 302, w = 75, label = "Create", parent = adverts}.DoClick = function()
	...
	RunConsoleCommand(
		"xgui",
		"addAdvert",
		adverts.message:GetValue(),
		(rpt < 0.1) and 0.1 or rpt,
		adverts.group:GetValue(),
		col.r,
		col.g,
		col.b,
...
adverts.updatebutton.DoClick = function(node)
	...
	if (((type(node.group) == "number") and "<No Group>" or node.group) == adverts.group:GetValue()) then
		RunConsoleCommand(
			...
			node.number,
			adverts.message:GetValue(),
			(adverts.time:GetValue() < 0.1) and 0.1 or adverts.time:GetValue(),
			col.r,
			col.g,
			col.b,
			adverts.csay:GetExpanded() and adverts.display:GetValue() or nil
...
adverts.nodeup =
	...
	y = 302,
	w = 20,
	icon = "icon16/bullet_arrow_up.png",
	centericon = true,
	parent = adverts,
	disabled = true
}
...
adverts.nodedown =
	...
	y = 302,
	w = 20,
	icon = "icon16/bullet_arrow_down.png",
	centericon = true,
	parent = adverts,
	disabled = true
}
...
function adverts.removeAdvert(node)
	if node then
		Derma_Query(
			"Are you sure you want to delete this " .. (node.data and "advert?" or "advert group?"),
			"XGUI WARNING",
			"Delete",
			function()
...
function adverts.RenameAdvert(old)
	advertRename =
		xlib.makeframe {
		label = "Set Name of Advert Group - " .. old,
		w = 400,
		h = 80,
		showclose = true,
		skin = xgui.settings.skin
	}
...
function adverts.updateAdverts()
	...
	table.sort(
		sortSingle,
		function(a, b)
			return string.lower(a.message) < string.lower(b.message)
		end
	)
	table.sort(
		sortGroups,
	...
	for _, advert in ipairs(sortSingle) do
		adverts.createNode(
			adverts.tree,
			xgui.data.adverts[advert.group][1],
			advert.group,
			1,
			xgui.data.adverts[advert.group][1].message,
			lastNode
		)
	end
	for _, group in ipairs(sortGroups) do
		...
		if groupStates[group] then
			foldernode:SetExpanded(true, true)
		end
	end
	...
	if node then
		...
		if adverts.isBottomNode(node) then
			adverts.nodedown:SetDisabled(true)
		end
...
plist.variablePicker =
	xlib.makecombobox {
	choices = {
		"Banned By - Admin:SteamID who created the ban",
		"Ban Start - Date/Time the ban was created",
		"Reason",
		"Time Left",
		"SteamID (excluding non-number characters)",
		"SteamID64 (useful for constructing URLs for appealing bans)"
	},
	zpos = 4
}
...
xgui.handleBanPreview = function(message)
	...
	local message =
		...
		y = 35,
		label = message,
		textcolor = Color(191, 191, 191, 255),
		font = "DefaultLarge",
		parent = preview
	}
...
plist:Add(
	xlib.makecombobox {
		repconvar = "ulx_logEcho",
		isNumberConvar = true,
		choices = {"Do not echo admin commands", "Echo admin commands anonymously", "Echo commands and identify admin"}
	}
)
plist:Add(
	xlib.makecombobox {
		repconvar = "ulx_logSpawnsEcho",
		isNumberConvar = true,
		choices = {"Do not echo spawns", "Echo spawns to admins only", "Echo spawns to everyone"}
	}
)
...
plist:Add(
	xlib.makecombobox {
		repconvar = "ulx_meChatEnabled",
		isNumberConvar = true,
		choices = {"Disabled", "Sandbox Only", "Enabled"}
	}
)
plist:Add(xlib.makelabel {label = "\nWelcome Message"})
...
plist:Add(
	xlib.makeslider {
		...
		max = 600,
		decimal = 0,
		repconvar = "ulx_kickAfterNameChangesCooldown"
	}
)
plist:Add(
	xlib.makecheckbox {label = "Warn players how many name-changes remain", repconvar = "ulx_kickAfterNameChangesWarning"}
)
...
plist:Add(
	xlib.makecombobox {
		repconvar = "ulx_showmotd",
		isNumberConvar = true,
		choices = {"0 - Disabled", "1 - Local File", "2 - MOTD Generator", "3 - URL"},
		zpos = 1
	}
)
...
pnlFontServerName.name =
	...
	selectall = true,
	choices = commonFonts,
	parent = pnlFontServerName
}
pnlFontServerName.size =
	xlib.makeslider {
	x = 5,
	y = 30,
	w = 250,
...
pnlFontSubtitle.name =
	...
	selectall = true,
	choices = commonFonts,
	parent = pnlFontSubtitle
}
pnlFontSubtitle.size =
	xlib.makeslider {
	x = 5,
	y = 30,
	w = 250,
...
plist.updateGeneratorSettings = function(data)
	if not data then
		data = xgui.data.motdsettings
	end
	if not data or not data.style or not data.info then
		return
	end
	if not plist.generator:IsVisible() then
		return
	end
	...
	for i = 1, #data.info do
		...
		btnRemove.DoClick = function()
			Derma_Query(
				'Are you sure you want to remove the section "' .. section.title .. '"?',
				"XGUI WARNING",
				"Remove",
				function()
...
panel.remainingmaps =
	xlib.makelistview {x = 140, y = 20, w = 135, h = 262, multiselect = true, headerheight = 0, parent = panel}
...
panel.votemapmode =
	...
	y = 302,
	w = 275,
	repconvar = "ulx_votemapMapmode",
	isNumberConvar = true,
	numOffset = 0,
	choices = {"Include new maps by default", "Exclude new maps by default"},
	parent = panel
}
...
plist:Add(
	xlib.makecombobox {
		repconvar = "ulx_rslotsMode",
		isNumberConvar = true,
		choices = {
			"0 - Reserved slots disabled",
			"1 - Admins fill slots",
			"2 - Admins don't fill slots",
			"3 - Admins kick newest player"
		}
	}
)
plist:Add(
	xlib.makeslider {label = "Number of Reserved Slots", min = 0, max = game.MaxPlayers(), repconvar = "ulx_rslots"}
)
