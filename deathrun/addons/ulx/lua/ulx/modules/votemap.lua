...
function ulx.votemap(calling_ply, map)
	...
	ULib.tsay(
		_,
		string.format(
			'%s voted for %s (%i/%i). Say "!votemap %i" to vote for this map too.',
			calling_ply:Nick(),
			ulx.votemaps[mapid],
			mapvotes[mapid],
			votes_needed,
	...
	ulx.logString(
		string.format("%s voted for %s (%i/%i)", calling_ply:Nick(), ulx.votemaps[mapid], mapvotes[mapid], votes_needed)
	)
	...
	if mapvotes[mapid] >= votes_needed then
		...
		if #admins <= 0 or vetotime < 1 then
			...
			ULib.tsay(
				_,
				"Vote for map " .. ulx.votemaps[mapid] .. " successful! Now pending admin approval. (" .. vetotime .. " seconds)",
				true
			) -- TODO, color?
