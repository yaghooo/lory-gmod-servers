...
function xgui.load_helpers()
	...
	function xgui.makeXGUIbase()
		...
		xgui.base.animOpen = function()
			--First 4 are fade animations, last (or invalid choice) is the default fade animation.
			xgui.settings.animIntype = tonumber(xgui.settings.animIntype)
			if xgui.settings.animIntype == 2 then
				xlib.addToAnimQueue(
					function()
						xgui.anchor:SetAlpha(255)
					end
				)
				xlib.addToAnimQueue(
					"pnlSlide",
		...
		function xgui.base:SetActiveTab(active, ignoreAnim)
			if (self.m_pActiveTab == active) then
				return
			end
		...
		--Progress bar
		xgui.chunkbox =
			xlib.makeprogressbar {x = 420, w = 180, h = 20, visible = false, skin = xgui.settings.skin, parent = xgui.anchor}
		function xgui.chunkbox:Progress(datatype)
			self.value = self.value + 1
			self:SetFraction(self.value / self.max)
			self.Label:SetText(
				"Getting data: " .. datatype .. " - " .. string.format("%.2f", (self.value / self.max) * 100) .. "%"
			)
	...
	function ULib.cmds.NumArg.x_getcontrol(arg, argnum, parent)
		...
		if table.HasValue(arg, ULib.cmds.allowTimeString) then
			...
			outPanel.GetRawValue = function(self)
				...
				if char == "m" or char == "p" or tonumber(self.val:GetValue()) == 0 then
					char = ""
				end
			...
			if table.HasValue(arg, ULib.cmds.optional) then
				defvalue = arg.default
			end
			if not defvalue then
				defvalue = 0
			end --No default was set for this command, so we'll use 0.
			...
			outPanel.GetValue = function(self)
				return outPanel.val.GetValue(outPanel.val)
			end
			outPanel.TextArea = outPanel.val.TextArea
			return outPanel
		end
	...
	function ULib.cmds.NumArg.getTime(arg)
		...
		for i, v in ipairs(charPriority) do
			if arg:find(v, 1, true) then
				if not charMap[i] or not divisor[i] or not ULib.stringTimeToMinutes(arg) then
					return nil, nil
				end
	...
	function ULib.cmds.StringArg.x_getcontrol(arg, argnum, parent)
		...
		local is_restricted_to_completes =
			table.HasValue(arg, ULib.cmds.restrictToCompletes) or restrictions.playerLevelRestriction -- Program-level restriction (IE, ulx map) -- The player's tag specifies only certain strings

		if is_restricted_to_completes then
			return xlib.makecombobox {
				text = arg.hint or "StringArg",
				choices = restrictions.restrictedCompletes,
				parent = parent
			}
	...
	function ULib.cmds.BoolArg.x_getcontrol(arg, argnum, parent)
		...
		if restrictions.restrictedTo ~= nil then
			outPanel:SetDisabled(true)
		end
		outPanel.GetValue = function(self)
