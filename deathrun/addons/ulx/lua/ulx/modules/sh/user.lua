...
function ulx.adduserid(calling_ply, id, group_name)
	...
	if not checkForValidId(calling_ply, id) then
		return false
	end
...
function ulx.userallow(calling_ply, target_ply, access_string, access_tag)
	...
	if not success then
		ULib.tsayError(
			calling_ply,
			string.format('User "%s" already has access to "%s"', target_ply:Nick(), access_string),
			true
		)
	else
...
function ulx.userallowid(calling_ply, id, access_string, access_tag)
	...
	local success = ULib.ucl.userAllow(id, accessTable)
	local name =
		(ULib.ucl.authed[id] and ULib.ucl.authed[id].name) or (ULib.ucl.users[id] and ULib.ucl.users[id].name) or id
	if not success then
...
function ulx.userdeny(calling_ply, target_ply, access_string, should_use_neutral)
	...
	if should_use_neutral then
		...
		if not success then
			ULib.tsayError(
				calling_ply,
				string.format('User "%s" is already denied access to "%s"', target_ply:Nick(), access_string),
				true
			)
		else
...
userdeny:addParam {
	...
	hint = "remove explicit allow or deny instead of outright denying",
	ULib.cmds.optional
}
...
function ulx.userdenyid(calling_ply, id, access_string, should_use_neutral)
	...
	if should_use_neutral then
		if success then
			...
			ULib.tsayError(
				calling_ply,
				string.format('User "%s" isn\'t denied or allowed access to "%s"', name, access_string),
				true
			)
		end
	else
...
local setgroupcantarget =
	ulx.command(CATEGORY_NAME, "ulx setgroupcantarget", ulx.setGroupCanTarget, nil, false, false, true)
setgroupcantarget:addParam {
	type = ULib.cmds.StringArg,
	completes = ulx.group_names,
	hint = "group",
	error = 'invalid group "%s" specified',
