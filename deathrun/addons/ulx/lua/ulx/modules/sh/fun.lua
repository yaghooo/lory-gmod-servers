	...
	for i = 1, #target_plys do
		...
		elseif v:IsFrozen() then
			...
			timer.Create(
				...
				0,
				function()
					-- Repeat forever, we have an unhooker inside.
...
doJail = function(v, seconds)
	...
	local function unjail()
		...
		if not v:IsValid() then
			return
		end -- Make sure they're still connected
...
local function ragdollSpawnCheck(ply)
	if ply.ragdoll then
		timer.Simple(
			0.01,
			function()
				-- Doesn't like us using it instantly
				if not ply:IsValid() then
					return
				end -- Make sure they're still here
	...
	for i = 1, #players do
		...
		if ply.ragdollAfterCleanup then
			...
			timer.Simple(
				0.1,
				function()
					-- Doesn't like us re-creating the ragdoll immediately
					ragdollPlayer(ply)
				end
			)
		end
...
zombieDeath = function(ent, ply)
	if ply.maul_npcs then -- Recreate!
		...
		ULib.queueFunctionCall(
			function()
				-- Create it next frame because 1. Old NPC won't be in way and 2. We won't overflow the server while shutting down with someone being mauled
				if not ply:IsValid() then
					return
				end -- Player left
