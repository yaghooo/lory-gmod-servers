...
function atlaschat.CreateFont(name, unique, font, size, weight, antialias)
    surface.CreateFont(
        unique,
        {font = font, size = size, weight = weight, antialias = antialias == nil and true or antialias}
    )
    surface.CreateFont(
        unique .. ".shadow",
        {font = font, size = size, weight = weight, antialias = false, outline = true, blursize = 1}
    )
...
function theme:Initialize()
    local w, h, x, y =
        atlaschat.ScaleSize(200, true),
        atlaschat.ScaleSize(150),
        atlaschat.ScaleSize(10, true),
        atlaschat.ScaleSize(230)
    ...
    if (atlaschat.chat_x:GetInt() == -1) then
        atlaschat.chat_x:SetInt(x)
    end
    if (atlaschat.chat_y:GetInt() == -1) then
        atlaschat.chat_y:SetInt(y)
    end
    if (atlaschat.chat_w:GetInt() == 0) then
        atlaschat.chat_w:SetInt(w)
    end
    if (atlaschat.chat_h:GetInt() == 0) then
        atlaschat.chat_h:SetInt(h)
    end
    ...
    self.settingsIcon =
        panel:AddIcon(
        "atlaschat/settings.png",
        function()
            atlaschat.theme.Call("ToggleConfigPanel")
        end,
        "Configuration"
    )

    self.informationIcon =
        panel:AddIcon(
        "atlaschat/emotes.png",
        function()
            if not IsValid(self.expressionPanel) then
...
function theme:PaintGenericBackground(panel, w, h, text, x, y, xAlign, yAlign)
    ...
    if text then
        draw.SimpleText(
            text,
            "atlaschat.theme.default.title",
            x or 6,
            y or 8,
            self.color.generic_label,
            xAlign or TEXT_ALIGN_LEFT,
            yAlign or TEXT_ALIGN_BOTTOM
        )
    end
end
...
function theme:PaintPanel(w, h)
    ...
    if IsValid(self.panel.hostName) and self.panel.hostName:GetText() ~= hostName then
...
function theme:PaintTextEntry(w, h, entry)
    ...
    entry:DrawTextEntryText(
        self.color.generic_label,
        entry.m_colHighlight or entry:GetSkin().colTextEntryTextHighlight,
        self.color.generic_label
    )
end
...
local function bool(v)
    return v == true or v == false or v == "true" or v == "false"
end
...
for name, object in pairs(config) do
    if object:GetText() and not object.server then
        ...
        if not isBool and not isNumber then
            index = 3
        end
...
function theme:ToggleConfigPanel()
    if not IsValid(self.config) then
        ...
        if LocalPlayer():IsSuperAdmin() then
            self.config:AddButton(
                "Reset everyone's configuration",
                true,
                function()
                    net.Start("atlaschat.rqclrcfg")
                    net.SendToServer()
...
function theme:ToggleUserList()
    if not IsValid(self.userListBase) then
...
function theme:ToggleRankMenu()
    if not IsValid(self.rankMenu) then
...
function theme:OnToggle(show)
    ...
    if show then
        ...
        if atlaschat.chat_x:GetInt() == -1 then
            x = atlaschat.ScaleSize(10, true)
            atlaschat.chat_x:SetInt(x)
        end
        if atlaschat.chat_y:GetInt() == -1 then
            y = atlaschat.ScaleSize(230)
            atlaschat.chat_y:SetInt(y)
        end
        if atlaschat.chat_w:GetInt() == 0 then
            w = atlaschat.ScaleSize(200, true)
            atlaschat.chat_w:SetInt(w)
        end
        if atlaschat.chat_h:GetInt() == 0 then
            h = atlaschat.ScaleSize(150)
            atlaschat.chat_h:SetInt(h)
        end
        ...
        for k, child in pairs(children) do
            if IsValid(child) and (child.fade and child.fade <= CurTime()) or not child.fade then
                child:SetAlpha(0)
            end
        end
    end
    ...
    if IsValid(self.userListBase) and self.userListBase:IsVisible() then
...
function theme:PlayerLabelPressed(label)
    ...
    menu:AddOption(
        ...
        function()
            SetClipboardText(label.steamID)
        end
    )

    if LocalPlayer():IsSuperAdmin() or (label.steamID == LocalPlayer():SteamID()) then
        menu:AddSpacer()
        ...
        for i = 1, #data do
            ...
            if type == "string" or type == "number" then
                ...
                if firstType then
                    ...
                    local canUse =
                        title and true or
                        hook.Run("AtlasChatCanUseExpression", player, expression, firstType:GetUnique())
                    local panelObject
    ...
    for i = 1, #data do
        ...
        if type == "Player" then
            ...
            if avatarsEnabledLocal and avatarsEnabled then
                local canUse =
                    hook.Run("AtlasChatCanUseExpression", value, "<avatar=" .. value:SteamID() .. ">", "avatar_steamid")
            ...
            local color, text =
                data[i - 1] and IsColor(data[i - 1]) and realColor or team.GetColor(value:Team()),
                value:Nick() --GAMEMODE.GetAmMurderer and value:Team() == 2 and value:GetBystanderName() or value:Nick()
        ...
        if type == "Panel" then
            ...
            if value:GetClassName() == "Label" then
                ...
                local exploded, start, ending, color, seperator =
                    string.Explode(" ", valueText),
                    1,
                    1,
                    value:GetTextColor(),
                    " "
    ...
    if self.panel:IsVisible() then
        if list:GetShouldScroll() then
            timer.Simple(
                0.1,
                function()
                    list.VBar:SetScroll(list:GetCanvas():GetTall())
                end
            )
        end
    else
