...
function panel:RemoveIcon(icon)
    ...
    timer.Simple(
        0,
        function()
            self.iconHolder:InvalidateLayout(true)
            self.iconHolder:Resize()
        end
    )
end
...
function panel:ChangeList(key, list)
    if IsValid(self.activeList) then
    ...
    if key then
        if not IsValid(self.userList) then
            self.userList =
                self:AddIcon(
                "atlaschat/users.png",
                function()
                    atlaschat.theme.Call("ToggleUserList")
                end,
                "Chat Userlist"
            )
        end
...
function panel:Think()
    ...
    if self.Dragging[1] ~= 0 then
        ...
        if IsValid(userListBase) and userListBase:IsVisible() then
