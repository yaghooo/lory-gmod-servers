...
function panel:ThrowIntoBlackHole()
    ...
    timer.Simple(
        ...
        function()
            ...
            parent:GetParent().scrollbar:OnScroll(1)
        end
    )
end
...
function panel:RemovePlayer(player, left)
    ...
    if player == LocalPlayer() then
        ...
        if left then
            ...
            atlaschat.theme.Call(
                "ParseText",
                list,
                color_red,
                ":exclamation: " .. nick .. " was kicked from the chatroom!"
            )
        end
    ...
    if vertical then
        ...
        for k, panel in pairs(self.panels) do
            if IsValid(panel) then
                ...
                panel:SetPos(
                    panel.x,
                    panel.origin_y + (panel:GetTall() - (parent:GetTall() - panel.origin_y)) * scroll * -1
                )
            end
        end
    else
        ...
        for k, panel in pairs(self.panels) do
            if IsValid(panel) then
                ...
                panel:SetPos(
                    panel.origin_x + (panel:GetWide() - (parent:GetWide() - panel.origin_x)) * scroll * -1,
                    panel.y
                )
            end
        end
