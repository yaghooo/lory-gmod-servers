...
function ULib.slap(ent, damage, power, nosound)
	if ent:GetMoveType() == MOVETYPE_OBSERVER then
		return
...
local function checkBan(steamid64, ip, password, clpassword, name)
	...
	if not banData.admin and not banData.reason and not banData.unban and not banData.time then
		return
	end
...
function ULib.kick(ply, reason, calling_ply)
	local nick =
		calling_ply and calling_ply:IsValid() and
...
function ULib.invisible(ply, bool, visibility)
	if not ply:IsValid() then
		return
	end -- This is called on a timer so we need to verify they're still connected
