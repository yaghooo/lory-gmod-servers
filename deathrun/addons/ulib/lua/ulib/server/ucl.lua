...
function ucl.addGroup(name, allows, inherit_from, from_CAMI)
	...
	if ucl.groups[name] then
		return error("Group already exists, cannot add again (" .. name .. ")", 2)
	end
	if inherit_from then
		if inherit_from == name then
			return error("Group cannot inherit from itself", 2)
		end
		if not ucl.groups[inherit_from] then
			return error("Invalid group for inheritance (" .. tostring(inherit_from) .. ")", 2)
		end
	end

	-- Lower case'ify
	for k, v in ipairs(allows) do
		allows[k] = v:lower()
	end
...
function ucl.groupAllow(name, access, revoke)
	...
	if not ucl.groups[name] then
		return error("Group does not exist for changing access (" .. name .. ")", 2)
	end
...
function ucl.renameGroup(orig, new)
	...
	if orig == ULib.ACCESS_ALL then
		return error("This group (" .. orig .. ") cannot be renamed!", 2)
	end
	if not ucl.groups[orig] then
		return error("Group does not exist for renaming (" .. orig .. ")", 2)
	end
	if ucl.groups[new] then
		return error("Group already exists, cannot rename (" .. new .. ")", 2)
	end
...
function ucl.setGroupInheritance(group, inherit_from, from_CAMI)
	...
	if group == ULib.ACCESS_ALL then
		return error("This group (" .. group .. ") cannot have its inheritance changed!", 2)
	end
	if not ucl.groups[group] then
		return error("Group does not exist (" .. group .. ")", 2)
	end
	if inherit_from and not ucl.groups[inherit_from] then
		return error("Group for inheritance does not exist (" .. inherit_from .. ")", 2)
	end
	...
	while groupCheck do
		if groupCheck == group then -- Got back to ourselves. This is bad.
			...
			error(
				'Changing group "' ..
					group .. '" inheritance to "' .. inherit_from .. '" would cause cyclical inheritance. Aborting.',
				2
			)
		end
	...
	if old_inherit == inherit_from then
		return
...
function ucl.setGroupCanTarget(group, can_target)
	...
	if ucl.groups[group].can_target == can_target then
		return
	end -- Nothing to change
...
function ucl.removeGroup(name, from_CAMI)
	...
	if name == ULib.ACCESS_ALL then
		return error("This group (" .. name .. ") cannot be removed!", 2)
	end
	if not ucl.groups[name] then
		return error("Group does not exist for removing (" .. name .. ")", 2)
	end

	local inherits_from = ucl.groupInheritsFrom(name)
	if inherits_from == ULib.ACCESS_ALL then
		inherits_from = nil
	end -- Easier
...
function ucl.addUser(id, allows, denies, group, from_CAMI)
	...
	allows = allows or {}
	denies = denies or {}
	if allows == ULib.DEFAULT_GRANT_ACCESS.allow then
		allows = table.Copy(allows)
	end -- Otherwise we'd be changing all guest access
	if denies == ULib.DEFAULT_GRANT_ACCESS.deny then
		denies = table.Copy(denies)
	end -- Otherwise we'd be changing all guest access
	if group and not ucl.groups[group] then
		return error("Group does not exist for adding user to (" .. group .. ")", 2)
	end
	...
	for k, v in ipairs(denies) do
		denies[k] = v
	end

	local name, oldgroup
	if ucl.users[id] and ucl.users[id].name then
		name = ucl.users[id].name
	end -- Preserve name
	if ucl.users[id] and ucl.users[id].group then
		oldgroup = ucl.users[id].group
	end
...
function ucl.userAllow(id, access, revoke, deny)
	...
	if not userInfo then
		return error("User id does not exist for changing access (" .. id .. ")", 2)
	end
...
function ucl.removeUser(id, from_CAMI)
	...
	if not userInfo then
		return error("User id does not exist for removing (" .. id .. ")", 2)
	end
...
function ucl.registerAccess(access, groups, comment, category)
	...
	if accessStrings[access] ~= comment then -- Only if not already registered or if the comment has changed
		...
		for _, groupInfo in pairs(ucl.groups) do
			if table.HasValue(groupInfo.allow, access) then
				return
			end -- Found, don't add again
		end
