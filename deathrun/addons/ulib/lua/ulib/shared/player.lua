	...
	if not trace_results.Entity:IsValid() or not trace_results.Entity:IsPlayer() then
		...
		for _, player in ipairs(players) do
			if player ~= ply then
				...
				local diff =
					math.abs(math.NormalizeAngle(newang.pitch - ang.pitch)) + math.abs(math.NormalizeAngle(newang.yaw - ang.yaw))
...
function ULib.getUser(target, enable_keywords, ply)
	...
	elseif #plyMatches > 1 then
		...
		return false, "Found multiple targets: " ..
			str .. ". Please choose a better string for the target. (EG, the whole name)"
	end
