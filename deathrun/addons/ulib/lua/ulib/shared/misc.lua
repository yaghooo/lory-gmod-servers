...
function ULib.stripComments(str, comment, blockcommentbeg, blockcommentend)
	if blockcommentbeg and string.sub(blockcommentbeg, 1, string.len(comment)) == comment then -- If the first of the block comment is the linecomment ( IE: --[[ and -- ).
		string.gsub(
			str,
			ULib.makePatternSafe(comment) .. "[%S \t]*",
			function(match)
...
function ULib.parseKeyValues(str, convert)
	...
	if convert and table.Count(current_table) == 1 and type(current_table.Out) == "table" then -- If we caught a stupid garry-wrapper
...
function ULib.makeKeyValues(t, tab, completed)
	...
	if completed[t] then
		return ""
	end -- We've already done this table.
	completed[t] = true
...
local function navigateUpTo(currentPointer, tableCrumbs)
	for i = 1, #tableCrumbs - 1 do
		...
		if type(currentPointer) ~= "table" then
			return false
		end -- Not found
	end
...
local function getCrumbsTable(varLocation)
	...
	for i = 1, #tableCrumbs do
		...
		if replaced > 0 then
			tableCrumbs[i] = tonumber(newCrumb)
		end
	end
	return tableCrumbs
end
...
if type(meta) == "boolean" then
	return
end -- Metatable is protected, so we aren't able to run this code without erroring.
