...
local function ULibRPC()
	...
	if not success or type(fn) ~= "function" then
		return error("Received bad RPC, invalid function (" .. tostring(fn_string) .. ")!")
	end
...
local function readCvar(um)
	...
	ULib.queueFunctionCall(
		function()
			-- Queued to ensure we don't overload the client console
...
local function changeCvar(um)
	...
	ULib.queueFunctionCall(
		function()
			-- Queued so we won't overload the client console and so that changes are always going to be called via the hook AFTER the initial hook is called
			if changed then
