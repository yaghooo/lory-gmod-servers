...
if SERVER then
    ...
    hook.Add(
        "DeathrunPlayerFinishMap",
        "DeathrunMapRecords",
        function(ply, zname, zone, place, seconds)
            local sid64 = ply:SteamID64()
            local mapname = game.GetMap()

            sql.Query(
                "INSERT INTO deathrun_records VALUES ('" ..
                    sid64 .. "', '" .. mapname .. "', " .. tostring(seconds) .. ")"
            )
        end
    )
    ...
    hook.Add(
        ...
        function()
            ...
            res =
                sql.Query(
                "SELECT * FROM deathrun_records WHERE mapname = '" .. game.GetMap() .. "' ORDER BY seconds ASC LIMIT 3"
            )
            ...
            for k, ply in ipairs(player.GetAll()) do
                res2 =
                    sql.Query(
                    "SELECT * FROM deathrun_records WHERE mapname = '" ..
                        game.GetMap() .. "' AND sid64 = '" .. ply:SteamID64() .. "' ORDER BY seconds ASC LIMIT 3"
                )
    ...
    hook.Add(
        ...
        function(ply)
            ...
            if not res then
                res =
                    sql.Query(
                    "INSERT INTO deathrun_ids VALUES ( '" ..
                        id64 .. "', '" .. id .. "', '" .. Base64Encode(ply:Nick()) .. "' )"
                )
            else
                res =
                    sql.Query(
                    "UPDATE deathrun_ids SET nick = '" ..
                        Base64Encode(ply:Nick()) .. "' WHERE sid64 = '" .. id64 .. "' "
                )
            end
        end
    )
    ...
    function DR:SteamToNick(sid)
        local com = true
        if string.find(sid, "STEAM_") ~= nil then
            com = false
        end
    ...
    sql.Query(
        "CREATE TABLE deathrun_stats ( sid STRING, kills INTEGER, deaths INTEGER, runner_wins INTEGER, death_wins INTEGER )"
    )

    hook.Add(
        "PlayerAuthed",
        "CreateStatsRow",
        function(ply, steamid, uid)
            ...
            if ROUND:GetCurrent() == ROUND_ACTIVE then
                if att:IsPlayer() then
                    if vic:Team() ~= att:Team() then
                        ...
                        res =
                            sql.Query(
                            "UPDATE deathrun_stats SET kills = " ..
                                tostring(kills + 1) .. " WHERE sid = '" .. att:SteamID() .. "'"
                        )
                    end
                else
                    if vic:Team() == TEAM_RUNNER then
                        for _, ply in ipairs(team.GetPlayers(TEAM_DEATH)) do
                            if not ply:IsBot() then
                                data1 =
                                    sql.Query("SELECT kills FROM deathrun_stats WHERE sid = '" .. ply:SteamID() .. "'")
                                local kills = data1[1]["kills"]
                                res =
                                    sql.Query(
                                    "UPDATE deathrun_stats SET kills = " ..
                                        tostring(kills + 1) .. " WHERE sid = '" .. ply:SteamID() .. "'"
                                )
                            end
                        end
                    end
                ...
                if vic:IsPlayer() and not vic:IsBot() then
                    ...
                    res =
                        sql.Query(
                        "UPDATE deathrun_stats SET deaths = " ..
                            tostring(deaths + 1) .. " WHERE sid = '" .. vic:SteamID() .. "'"
                    )
                end
            end
        end
            ...
            if winteam == TEAM_RUNNER or winteam == TEAM_DEATH then
                ...
                if winteam == TEAM_RUNNER then
                    for k, ply in ipairs(players) do
                        if not ply:IsBot() then
                            local data1 =
                                sql.Query(
                                "SELECT runner_wins FROM deathrun_stats WHERE sid = '" .. ply:SteamID() .. "'"
                            )
                            local wins = data1[1]["runner_wins"]
                            sql.Query(
                                "UPDATE deathrun_stats SET runner_wins = " ..
                                    tostring(wins + 1) .. " WHERE sid = '" .. ply:SteamID() .. "'"
                            )
                        end
                    end
                end
                if winteam == TEAM_DEATH then
                    for k, ply in ipairs(players) do
                        if not ply:IsBot() then
                            local data1 =
                                sql.Query("SELECT death_wins FROM deathrun_stats WHERE sid = '" .. ply:SteamID() .. "'")
                            local wins = data1[1]["death_wins"]
                            sql.Query(
                                "UPDATE deathrun_stats SET death_wins = " ..
                                    tostring(wins + 1) .. " WHERE sid = '" .. ply:SteamID() .. "'"
                            )
                        end
                    end
    ...
    function DR:DisplayStats(ply) -- displays a player's stats in front of their face
        if IsValid(ply) then
            ...
            local res2 =
                sql.Query(
                "SELECT sid, (runner_wins + death_wins) AS total_wins FROM deathrun_stats ORDER BY total_wins DESC LIMIT 1"
            )
            ...
            if t < lifetime then
                ...
                if t < lifetime - 1 then
                    ...
                    surface.DrawRect(
                        ...
                        y,
                        w,
                        h * QuadLerp(math.Clamp(InverseLerp(t, lifetime - 1, lifetime), 0, 1), 1, 0)
                    )
                end
                    ...
                    if DR.MapRecordsCache[1] ~= nil then
                        for i = 1, #DR.MapRecordsCache + 2 do
                            ...
                            if i <= #DR.MapRecordsCache then
                                ...
                                deathrunShadowTextSimple(
                                    tostring(i) .. ". " .. string.sub(v["nickname"] or "", 1, 24),
                                    "deathrun_3d2d_large",
                                    -700,
                                    -150 + 100 * k,
                                ...
                                deathrunShadowTextSimple(
                                    string.ToMinutesSecondsMilliseconds(v["seconds"] or "0"),
                                    "deathrun_3d2d_large",
                                    700,
                                    -150 + 100 * k,
                                    DR.Colors.Text.Turq,
                            ...
                            elseif i == #DR.MapRecordsCache + 2 and DR.MapPBCache ~= 0 then
                                ...
                                deathrunShadowTextSimple(
                                    string.ToMinutesSecondsMilliseconds(DR.MapPBCache or 0),
                                    "deathrun_3d2d_large",
                                    700,
                                    -150 + 100 * k,
                                    DR.Colors.Text.Turq,
