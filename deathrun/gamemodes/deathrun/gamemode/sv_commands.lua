...
concommand.Add(
    ...
    function(ply, cmd, args)
        if
            DR:CanAccessCommand(ply, cmd) and ply:Alive() and ply:GetObserverMode() == OBS_MODE_NONE and
                (stuckers[ply:SteamID64()] or 0) < CurTime() - GetConVar("deathrun_unstuck_cooldown"):GetInt()
         then
            local trace = {
                start = ply:EyePos(),
        ...
        if args[1] then
            ...
            if DR:CanAccessCommand(ply, cmd) then
                ...
                elseif #targets == 1 then
                    ...
                    DeathrunSafeChatPrint(
                        ply,
                        "Punishing " ..
                            tostring(t:Nick()) .. " for another " .. tostring(DR:GetDeathAvoid(t)) .. " rounds."
                    )
                elseif #targets > 1 then
