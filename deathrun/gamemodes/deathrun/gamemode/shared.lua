...
local defaultFlags =
    FCVAR_SERVER_CAN_EXECUTE + FCVAR_REPLICATED + FCVAR_NOTIFY + FCVAR_ARCHIVE + FCVAR_CLIENTCMD_CAN_EXECUTE
...
hook.Add(
    "SetupMove",
    "DeathrunDisableSpectatorSpacebar",
    function(ply, mv, cmd)
...
if SERVER then
    concommand.Add(
        "deathrun_internal_set_autojump",
...
function GM:Move(ply, data)
    ...
    if
        ply.AutoJumpEnabled == true and GetConVar("deathrun_allow_autojump"):GetBool() == true and
            GetConVar("deathrun_autojump_velocity_cap"):GetFloat() ~= 0
     then
...
local function AutoHop(ply, data)
    ...
    if ply.AutoJumpEnabled == false or GetConVar("deathrun_allow_autojump"):GetBool() == false then
        return
    end
