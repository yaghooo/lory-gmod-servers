...
local function checkdeathavoid(ply)
    ...
    if
        avoided == true and (ROUND:GetCurrent() == ROUND_PREP or ROUND:GetCurrent() == ROUND_ACTIVE) and
            #player.GetAllPlaying() > 2
     then
        DR:PunishDeathAvoid(ply, GetConVarNumber("deathrun_death_avoid_punishment"))
        DR:ChatBroadcast(
            "Player " .. ply:Nick() .. " will be punished for attempting to avoid being on the Death team!"
        )
    end
end
            ...
            while #deaths < deathsNeeded and timesLooped < 100 do
                if #punishmentpool > 0 then
                    ...
                    DR:ChatBroadcast(
                        "Player " ..
                            ply:Nick() ..
                                " is being punished for death avoidance! They have " ..
                                    tostring(DR:GetDeathAvoid(ply)) .. " Death rounds remaining."
                    )
...
ROUND:AddState(
    ...
    function()
        ...
        if SERVER then
            ...
            timer.Create(
                "DeathrunAutoslay",
                GetConVarNumber("deathrun_autoslay_delay") + 5,
                1,
                function()
                    for k, v in ipairs(player.GetAllPlaying()) do
                        ...
                        if idletime > GetConVarNumber("deathrun_autoslay_delay") then
                            ...
                            if v:Team() == TEAM_DEATH then
                                DR:ChatBroadcast(
                                    "Player " .. v:Nick() .. " went AFK during a Death round! They will be punished."
                                )
...
ROUND:AddState(
    ...
    function()
        ...
        if SERVER then
            if
                not hook.Call("DeathrunShouldMapSwitch", nil, rounds_played) and
                    (rounds_played < GetConVarNumber("deathrun_round_limit"))
             then
                DR:ChatBroadcast(
                    "Round " ..
                        tostring(rounds_played) ..
                            " over. " ..
                                tostring(GetConVarNumber("deathrun_round_limit") - rounds_played) .. " rounds to go!"
                )
                ROUND:SetTimer(GetConVarNumber("deathrun_finishtime_duration"))
                timer.Simple(
                    GetConVarNumber("deathrun_finishtime_duration"),
                    function()
                        ROUND:RoundSwitch(ROUND_PREP)
                    end
                )
...
if SERVER then
    ...
    function ROUND:FinishRound(winteam)
        ...
        DR:ChatBroadcast(
            "Round over! " ..
                (winteam == WIN_RUNNER and team.GetName(TEAM_RUNNER) .. " win!" or
                    winteam == WIN_DEATH and team.GetName(TEAM_DEATH) .. " win!" or
                    "Stalemate! Unbelievable!")
        )
        --calculate MVPs
        ...
        if mostkillsmvp and winteam == TEAM_RUNNER then
            table.insert(
                mvps,
                mostkillsmvp:Nick() .. " got " .. (mostkills) .. (mostkills > 1 and " kills!" or " kill!")
            )
        end
