    ...
    local function CalcViewThirdPerson(ply, pos, ang, fov, nearz, farz)
        if ThirdpersonOn:GetBool() == true and ply:Alive() and (ply:Team() ~= TEAM_SPECTATOR) then
            ...
            local tr =
                ...
                {
                    start = pos,
                    endpos = pos + ang:Forward() * -dist + Vector(0, 0, 9) + ang:Right() * ThirdpersonX:GetFloat() +
                        ang:Up() * ThirdpersonY:GetFloat(),
                    mins = Vector(-5, -5, -5),
                    maxs = Vector(5, 5, 5),
